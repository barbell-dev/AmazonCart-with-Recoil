...
function Element({ element, onRemove, onQuantityChange }) {
  const inputRef = useRef();

  const handleIncrement = () => {
    const newQuantity = element.quantity + 1;
    onQuantityChange(element.name, newQuantity);
  ...
  const handleInputChange = () => {
    if (inputRef.current.value.includes(".")) {
      alert("Cannot add decimal prices");
      return;
    }
  ...
  return (
    <div className="shopping-cart-element">
      ...
      <input
        type="number"
        defaultValue={element.quantity || 0}
        ref={inputRef}
        step={1}
        min={0}
...
function OrderSummary() {
  ...
  const orderSummaryElements = useRecoilValue(cartTotalSelector);

  return (
    <div className="flex flex-col">
      <h3>Order Summary</h3>
      <p>
        Items &nbsp; ({orderSummaryElements[0] ? orderSummaryElements[0] : "0"}
        ): &#8377;
        {orderSummaryElements[1]}
      </p>
    </div>
  );
}
